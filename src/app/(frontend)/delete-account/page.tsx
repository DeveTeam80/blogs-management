...
export default function DeleteAccountPage() {
  ...
  useEffect(() => {
    ...
    // Get all approved users except self for reassignment dropdown
    fetch('/api/users?where[status][equals]=approved&limit=100&depth=0')
      .then((r) => r.json())
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch('/api/users/delete-account', {
        ...
        body: JSON.stringify({
          reassignTo: reassignTo ? Number(reassignTo) : null,
        }),
      })
          ...
          {step === 'done' ? (
            ...
            <>
              ...
              <p className="da-sub">
                ...
                {reassignTo
                  ? ` Your posts will be transferred to the selected user.`
                  : ` Your posts will be marked as "Former Author".`}
              </p>
            ...
            <>
              ...
              <p className="da-sub">Before deleting, choose what happens to your posts.</p>
              <div className="da-divider" />

              <label className="da-label" htmlFor="reassign">
                Transfer posts to (optional)
              </label>
              <select
                id="reassign"
                className="da-select"
              ...
              >
                <option value="">— Don't transfer (mark as Former Author) —</option>
                {filteredUsers.map((u) => (
